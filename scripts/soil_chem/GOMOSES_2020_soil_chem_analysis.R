...
#last updated Jan. 30, 2020 by Steve Formel

#modified for GOMOSES 2020 poster figure
...
library(readxl)
samdf <- read_excel("Data/soil_chem/SF1_soil_chem_all_data.xls", 
                                    sheet = "data_cleaned")
...
df.post <- subset(samdf, samdf$time=="Post-Experiment")
...
autoplot(df.PCA, 
  ...
  geom_abline(intercept = 0.1, slope = 1.5, color="gray", 
              linetype="dashed", size=0.5) +
  theme_bw() +
  labs(title = "SF1 Post Experiment Soil Chem")
...
df.PCA <- prcomp(samdf[c(5:15)], center = TRUE, scale = TRUE)


#get group centroids
samdf$TT <- interaction(samdf$Treatment, samdf$time)
df.PCA.x <- as.data.frame(df.PCA$x)
df.PCA.x$groups <- samdf$TT
...
#Remake factors
pca.centroids$Soil <- rep(c("Live Soil", "Live Soil", "Autoclaved Soil", "Autoclaved Soil"), 2)
pca.centroids$Oil <- rep(c("No Oil", "Oiled"), 4)
pca.centroids$Time <- c(rep("Pre-Experiment", 4), rep("Post-Experiment", 4))
pca.centroids$Time <- factor(pca.centroids$Time, c("Pre-Experiment", "Post-Experiment"))
pca.centroids$Treatment <- interaction(pca.centroids$Soil, pca.centroids$Oil, sep = " , ")
pca.centroids$Treatment <- factor(pca.centroids$Treatment, c("Live Soil , No Oil", "Live Soil , Oiled", "Autoclaved Soil , No Oil", "Autoclaved Soil , Oiled"))
...
p1 <- p + 
  ...
  labs(title = "Soil Chemistry Over Time")
...
#save
ggsave(filename = "figs/GOMOSES_2020/Soil_chem_biplot_pre_to_post_exp.png", width = 9, height = 7, units = "in")

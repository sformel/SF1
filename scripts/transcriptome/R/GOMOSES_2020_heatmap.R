...
#modifieid for figure on GOMOSES 2020 poster
...
#it turns out that salmon estimates count so my numbers have decimals.  Therefore I have to import differently than Srah because I have to account for this using tximport example on: http://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html
...
annotation_colors = list(
  oil = c(Oiled="#D55E00", Not_Oiled="#56B4E9"),
  soil = c(Autoclaved="#F0E442", Live="black"))

pheatmap(counts_scale.genes_that_go_up_with_oil,
         show_rownames=F,
         annotation_col= samdf[,2:3],
         annotation_colors = annotation_colors,
         main = "DE Genes When Oil is Added",
         filename = "figs/GOMOSES_2020/heatmap_oil_added_genes.png", width = 9, height = 9)
...
#subset count data to autoclave gene isoforms
counts_scale.genes_in_both <- subset(counts_scale, rownames(counts) %in% both.DE)
...
pheatmap(counts_scale.genes_in_both,
         show_rownames=F,
         cluster_cols = FALSE,
         annotation_col= samdf[,2:3],
         annotation_colors = annotation_colors,

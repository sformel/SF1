...
s2c <- read.table("scripts/transcriptome/R/example_heatmap_from_SK//sample_table_RBFW_RNAseq.csv", header = TRUE, stringsAsFactors = TRUE, sep = ",")

s2c<-dplyr::filter(s2c,sampleName!="F84") #bad sample

s2c <- s2c %>% mutate(path = paste("star_DoCounts/",fileName, sep = ""))
...
#make a list of differentially expressed genes between dm and bm in liver, to subset counts dataframe
...
counts_liver_dm_bm_sig_genes<-counts[liver_dm_bm_sig_genes_list,26:37] #subset for just liver 

pheatmap(counts_liver_dm_bm_sig_genes)
pheatmap(counts_liver_dm_bm_sig_genes, annotation_col = column_annotation_samplephenotype)
...
counts_liver_dm_bm_sig_up_genes<-counts[liver_dm_bm_sig_up_genes_list,26:37] #subset for just liver 
